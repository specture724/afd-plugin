...
class AFDCUDAGraphPolicy:
    ...
    # Prefill token buckets for the AFD prefill FULL capture scheme. Empty
    # keeps every prefill step eager, which is the historical behavior; a
    # non-empty list layers bucketed prefill graphs on top of the upstream
    # FULL_DECODE_ONLY mode. The upstream mode name is unchanged because the
    # decode buckets must keep their existing FULL_DECODE_ONLY behavior.
    ...
    *,
    ...
    if not graph_enabled:
        return AFDCUDAGraphPolicy(
            ...
            prefill_buckets=resolve_prefill_buckets(),
        )
    ...
    # ### PATCH START: AFD prefill bucket policy resolution
    prefill_buckets = resolve_prefill_buckets()
    if prefill_buckets and role not in (None, "attention"):
        raise RuntimeError(
            "AFD prefill bucket capture is an Attention-side strategy; "
            f"role {role!r} must not set {PREFILL_BUCKETS_ENV}.",
        )
    # ### PATCH END: AFD prefill bucket policy resolution
